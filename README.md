...
This release contains source code, configs, tests, documentation, and Slurm
helper scripts. Large trained checkpoints, downloaded datasets, and full
experiment outputs are **not** bundled; they can be regenerated with the
commands below (or shared after the review period).
...
GPU / cluster users: install a matching PyTorch build first, then install this
package. See `requirements-canonical.txt` for a pinned reference stack used in
the paper runs.
...
- `scripts/slurm/` — cluster array / analysis entrypoints (set `PROJECT_DIR` to this repo)
...
Slurm scripts expect to be launched from the repository root. Before submitting,
export:
...
Replace any site-specific module loads with your cluster equivalents.
